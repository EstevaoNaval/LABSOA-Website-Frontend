<template>
  <div class="hidden lg:flex">
    <button class="flex btn btn-ghost m-auto" @click="sortStore.toggleSortDirection">
  ...
  <div class="flex lg:hidden">
    <button class="btn btn-sm btn-ghost flex m-auto" @click="sortStore.toggleSortDirection">
    ...
    <details class="dropdown dropdown-end m-auto" ref="sortOptionDropdownDetails">
      <summary class="text-md font-semibold text-center swap swap-rotate" @click="toggleSortOptionDropdown">
      ...
      <ul class="menu dropdown-content md:text-lg bg-base-100 rounded-box z-[1] p-2 shadow w-44">
        <li @click="handleSortOptionChoice(option.id)" v-for="option in sortStore.sortOptions" :key="option.id"><a>{{ option.name }}</a></li>
      </ul>
    </details>
  </div>
</template>
